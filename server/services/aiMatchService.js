// Lightweight rule-based NLP extraction for demo/mock mode.
// This keeps the AI Property Finder fully functional without any external API key.
// If ANTHROPIC_API_KEY + AI_PROVIDER=anthropic are set, extractQueryWithAnthropic() is used instead.
...
export const extractQueryWithAnthropic = async (text) => {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return extractQueryMock(text);
  ...
  try {
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "user",
            content: `Extract structured property search criteria from this query as strict JSON only (no prose, no markdown fences): {"purpose":"buy|rent","location":string|null,"propertyType":"house|apartment|plot|commercial"|null,"bedrooms":number|null,"budget":number|null (in PKR),"parkingRequired":boolean,"furnishedRequired":boolean}. Query: "${text}"`,
          },
        ],
      }),
    });
    ...
    return { ...JSON.parse(clean), raw: text };
  } catch (err) {
    console.error("Anthropic extraction failed, falling back to mock:", err.message);
    return extractQueryMock(text);
  }
};
